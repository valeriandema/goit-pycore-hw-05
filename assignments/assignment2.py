...
def generator_numbers(text: str) -> Generator[Decimal]:
    """
    Генератор, який приймає текст і шукає дійсні числа в тексті з допомогою регулярних виразів.
    Дійсні числа у тексті вважаються записаними без помилок і чітко відокремлені пробілами з обох боків.
    """
    # Регулярний вираз для пошуку дійсних чисел, відокремлених пробілами з обох боків
    pattern = r'(?<=\s)\d+\.\d+(?=\s)'
...
def sum_profit(text: str, func: Callable[[str], Generator[Decimal]]) -> Decimal:
    """
    Сумує дійсні числа, які знаходяться в тексті і повертаються генератором.
    Приймає функцію-генератор як параметр.
    """
